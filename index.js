...
async function showWalletBalances() {
  ...
  if (tokenAddress.trim() === '') {
    ...
    // Fetch token details for the first wallet (for name, symbol, etc.)
    ...
    const totalBalance = walletDetails.reduce((sum, details) => {
      if (!details.error) {
        return sum + parseFloat(details.balance);
      }
      return sum;
    }, 0).toFixed(tokenDetails.decimals); // Match decimals precision

    // Display token details without the 'balance' field
    console.log('\n--- Token Details ---', {
    ...
    // Display individual wallet balances
