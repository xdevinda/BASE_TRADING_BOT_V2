...
async function buyToken() {
  ...
  try {
    for (const fee of EXTENDED_FEE_TIERS) {
      ...
      if (poolAddress === ethers.ZeroAddress) continue; // Silently skip non-existent pools
    ...
    if (!priceInEth) {
      console.log('No liquidity pools found across all fee tiers. Cannot proceed with purchase.');
      return;
    }
  } catch (error) {
  ...
  const confirm = await askQuestion(`Confirm buying ${ethAmount} ETH worth of ${tokenDetailsBefore.symbol} with fee tier ${selectedFeeTier}? (y/n): `);
  ...
  try {
    const [token0, token1] = sortTokens(WETH_ADDRESS, tokenAddress);
    const poolAddress = await factory.getPool(token0, token1, selectedFeeTier);
    if (poolAddress === ethers.ZeroAddress) {
      console.log('Pool no longer exists. Aborting purchase.');
      return;
    }
    
    txHash = await executeSwap(wallets[0], WETH_ADDRESS, tokenAddress, amountIn, selectedFeeTier, provider, true);
    console.log('Transaction hash:', txHash);
    success = true;
  } catch (error) {
    console.log(`Purchase failed with fee tier ${selectedFeeTier}:`, error.message);
    return;
  }

  if (!success) {
    console.log('Could not complete purchase.');
    return;
  }
...
async function buyWithWallets() {
  ...
  await Promise.all(wallets.map(async (wallet, i) => {
    ...
    try {
      ...
      if (!priceInEth) {
        console.log(`No suitable fee tier found for Wallet ${i}. Skipping.`);
        return;
      }
    } catch (error) {
      console.error('Error fetching price:', error.message);
    ...
    try {
      ...
      const poolAddress = await factory.getPool(token0, token1, selectedFeeTier);
      if (poolAddress === ethers.ZeroAddress) {
        console.log(`Pool no longer exists for Wallet ${i}. Skipping.`);
        return;
      }
      txHash = await executeSwap(wallet, WETH_ADDRESS, tokenAddress, amountIn, selectedFeeTier, provider, true);
      console.log(`Wallet ${i} transaction hash: ${txHash}`);
      success = true;
    } catch (error) {
      console.log(`Buy failed for Wallet ${i} with fee tier ${selectedFeeTier}:`, error.message);
      return;
    }
    if (!success) {
      console.log(`Buy failed for Wallet ${i}.`);
      return;
    }
...
async function automateBuyAndSell() {
  ...
  await Promise.all(wallets.map(async (wallet, i) => {
    ...
    try {
      ...
      if (!priceInEth) {
        console.log(`No suitable fee tier found for initial buy for Wallet ${i}. Skipping.`);
        return;
      }
    } catch (error) {
      console.error(`Error fetching price for Wallet ${i}:`, error.message);
      return;
    }
    ...
    try {
      ...
      console.log(`Initial buy failed for Wallet ${i} with fee ${selectedFeeTier}:`, error.message);
    }
  }));
    ...
    if (isBuy) {
      ...
      if (ethBalance < amountIn + ((await getSafeGasPrice(provider)) * 50000n)) {
        ...
        try {
          ...
          if (!priceInEth) {
            console.log(`No suitable fee tier found for random buy for Wallet ${wallets.indexOf(wallet)}. Skipping.`);
            continue;
          }
        } catch (error) {
          console.error(`Error fetching price for Wallet ${wallets.indexOf(wallet)}:`, error.message);
          continue;
        }
        ...
        if (!dryRun) {
          ...
          try {
            ...
            console.log(`Random buy failed with fee ${selectedFeeTier} for Wallet ${wallets.indexOf(wallet)}:`, error.message);
          }
        } else {
      ...
      if (balance === 0n) {
        ...
        if (!dryRun) {
          let txHash;
          try {
            txHash = await executeSwap(wallet, tokenAddress, WETH_ADDRESS, amountToSell, selectedFeeTier, provider, false);
            ...
            console.log(`Sell failed with fee ${selectedFeeTier}:`, error.message);
          }
        } else {
          console.log(`Dry run: Simulating sell...`);
        }
